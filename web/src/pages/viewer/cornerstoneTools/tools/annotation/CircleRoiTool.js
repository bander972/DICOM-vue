...
export default class CircleRoiTool extends BaseAnnotationTool {
    ...
    for (let i = 0; i < toolState.data.length; i++) {
      ...
      if(this.pointNearTool(element,data,coords,interactionType)){
        ...
        external.cornerstone.updateImage(element);
                ...
                console.log("circleroi configuration",this.configuration)
  ...
  createNewMeasurement(eventData) {
    // console.log("生成新的测量数据",eventData);
    const goodEventData =
...
//绘制图像的函数
  renderToolData(evt) {
    // console.log("onImageRendered()-3 打印此时的---111target",evt.currentTarget,this.name);
    const toolData = getToolState(evt.currentTarget, this.name);
    // console.log(" circleroi 打印tooldata",toolData);
    if (!toolData) {
      return;
    }
    //查找画图的接口
    // console.log("查找画图的接口-renderToolData(evt)");
    ...
    draw(newContext, context => {
      ...
      if(data.text!=undefined){
         textBoxContent.push('Label:'+data.text);
      }else{
        textBoxContent.push('Label: undefined');
      }
