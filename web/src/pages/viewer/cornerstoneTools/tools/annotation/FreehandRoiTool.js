...
export default class FreehandRoiTool extends BaseAnnotationTool {
  constructor(props = {}) {
    ...
    //绑定鼠标双击修改标签属性
    this._changeTextCallback=this._changeTextCallback.bind(this);
  ...
  doubleClickCallback(evt) {
    console.log("freehandroi doubelclick");
    return this._updateTextForNearbyAnnotation(evt);  
  }
  //修改标签属性的回调函数(双击)
  _changeTextCallback(data, eventData, doneChangingTextCallback) {
      ...
      draw(context, context => {
        ...
        if (data.polyBoundingBox && !data.handles.textBox.freehand) {
          ...
          if(data.text!=undefined){
            text.push('Label:'+data.text);
         }else{
           text.push('Label: undefined');
         }
