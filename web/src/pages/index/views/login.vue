<template>
  <div class="login-wrap">
    <el-form label-position="left" :model="loginForm" :rules="rules" ref="ruleForm" class="demo-ruleForm login-container">
...
export default {
  ...
  methods: {
    test(){
      // var that = this;
      // this.$axios.post("http://localhost:8081/adm/login",this.ruleForm).then(function(response){
      //   alert(response.data);
      // })
    },
    //点击注册
    ...
    login(){  
      ...
      if(this.loginForm.uname==''||this.loginForm.upassword==''){
        ...
        this.$axios.post("http://localhost:8081/adm/login",this.loginForm).then(function(response){
        //console.log(response.data);
        if(response.data=="失败"){
         that.$message.error("用户名或密码有误！");
        }else{
          that.$store.commit('uptype',response.data);
          that.$store.commit('login', 'true');
          if(response.data=="idi"){
            that.$router.push({ path: '/setting/User' }); 
